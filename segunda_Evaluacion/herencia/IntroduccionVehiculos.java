...
public class IntroduccionVehiculos {
	...
	public static Camiones introduccionCamion(Scanner key) {
		...
		}while(!camion.getMatricula().matches("//d{4}[A-Z]{9}")||!camion.getMatricula().matches("[A-Z]{2} //d{5}")) ;
		do {
	 ...
	 * @return
	 */
	public static Turismos introducirTurismo(Scanner key) {
		boolean control = false;
		...
		}while(!turismo.getMatricula().matches("^[0-9]{4}[A-Z]{3}")|| !turismo.getMatricula().matches("^[A-Z]{2} [0-9]{5}"));
		do {
